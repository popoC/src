...
#include "po_dynamixel_2/po_control.h"
    ...
    :node_handle_("")
{
  ...
  uint8_t scan_range        = node_handle_.param<int>("scan_range", 6);
  ...
  ROS_INFO("scan_range %d",scan_range);
  ROS_INFO("dxl_baud_rate %d",dxl_baud_rate);
  ROS_INFO("profile_velocity %d",profile_velocity);
  ROS_INFO("profile_acceleration %d",profile_acceleration);
...
{
  printf("-----------------------------------------------------------------------\n");
  printf(" po2 dynamixel_workbench controller; position control example             \n");
  printf("-----------------------------------------------------------------------\n");
  printf("\n");
                                              ...
                                              dynamixel_workbench_msgs::JointCommand::Response &res)
{
  ...
  else if (req.unit == "wo")
  {
       ...
       dxl_wb_->itemWrite(req.id, "CW_Angle_Limit", 0);
       dxl_wb_->itemWrite(req.id, "CCW_Angle_Limit", 1023);
 
       printf("  ww test             \n");
       ret = dxl_wb_->goalPosition(req.id, req.goal_position);
       
  }
  else if (req.unit == "po")
  {
       printf("  popopopopopo test             \n");
 
       dxl_wb_->itemWrite(req.id, "CW_Angle_Limit", 0);
       dxl_wb_->itemWrite(req.id, "CCW_Angle_Limit", 0);
 
       printf("  popopopopopo2 test             \n");
 
       dxl_wb_->itemWrite(req.id, "Moving_Speed", req.goal_position);
       //driver_.writeRegister(id, "Moving_Speed", vel);
  }
  else if (req.unit == "po2")
  {
       printf("  test             \n");
